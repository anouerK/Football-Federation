...
///methode supprimer
     ...
     */
    public function deletejoueur (Request $request, SerializerInterface  $serializer)
    {
        $id = $request->get("cin");
        //$m = $this->getDoctrine();
     ...
     */
    public function  addJoueur1 (Request $request, NormalizerInterface $Normalizer){
        $em=$this->getDoctrine()->getManager();
        ...
        $reclamation->setAge($request->get("age"));
        $reclamation->setNbm($request->get("nbm"));
        $reclamation->setNba($request->get("nba"));
        $reclamation->setNationalite($request->get("natio"));
        $reclamation->setNumt($request->get("numt"));
    ...
    /**
     * @Route("/updateJoueur/{cin}", name="updateJoueur")
     */
    public function updateJoueur(Request $request, NormalizerInterface $Normalizer,$cin){
        $em=$this->getDoctrine()->getManager();
        $event=$em->getRepository(Joueur::class)->find($cin);

        $event->setnom($request->get('nom'));
        $event->setposte($request->get('poste'));
        $event->setPrenom($request->get("prenom"));
        $event->setAge($request->get("age"));
        $event->setNbm($request->get("nbm"));
        $event->setNba($request->get("nba"));
        $event->setNationalite($request->get("natio"));
        $event->setNumt($request->get("numt"));
        $event->setPhoto($request->get("photo"));
        // $event->setLogo($request->get('logo'));
        //$event->setPresident($request->get('president'));
        $em->flush();
        $jsonContent=$Normalizer->normalize($event,'json',['groups'=>'joueur']);
