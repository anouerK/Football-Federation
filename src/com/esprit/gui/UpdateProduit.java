...
marques marque;
    ...
    public UpdateProduit(Form f , produit p)
...

    {
     ...
     //   filePath= p.getImg();
        ...
        co1=p.getCouleur();
        taille1=p.getTaille();
        taille2=p.getTaille2();
        
          
         String qte = String.valueOf(p.getQte());
         String prix = String.valueOf(p.getPrix());
         TextField tfName=new TextField(p.getNom(),"Nom");
          TextField tfdesc=new TextField(p.getDesc(),"Description");
          TextField tfqte=new TextField(qte,"Quantite");
          TextField tfprix=new TextField(prix,"Prix");
        tfName.getAllStyles().setFgColor(0x607D8B);
       tfdesc.getAllStyles().setFgColor(0x607D8B);
         tfqte.getAllStyles().setFgColor(0x607D8B);
           tfprix.getAllStyles().setFgColor(0x607D8B);
         ...
         // CheckBox  cbcolor= new CheckBox("noire");
          // CheckBox  cbtaille1= new CheckBox("S");
           ...
           // TextField tfmarque=new TextField("","marque");
             ...
             Button btnAdd = new Button("Update Product");
             
            // System.out.println("marque::::"+p.getMar().getNomM());
             
             MultiButton b = new MultiButton(p.getCat().getTypec());
              MultiButton b2 = new MultiButton(p.getMar().getNomM());
               MultiButton b3 = new MultiButton(p.getTaille());
                MultiButton b4 = new MultiButton(p.getTaille2());
                 MultiButton b5 = new MultiButton(p.getCouleur());
            ...
            if (tfprix.getText() == "")
        {
          ...
          else
            {
              ...
              //  if(cbcolor.isSelected())ch3+="noire";
               ...
               produit badge = new produit(p.getId(),tfName.getText(),taille1,taille2,tfdesc.getText(),filePath,co1,prix,qte,categorie,marque);
              if(ServiceProduits.getInstance().updatep(badge)){
                   Dialog.show("Success","Produit Updated","OK",null);
               }
               else
               {
