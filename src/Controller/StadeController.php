...
/**
     ...
     */
    public function updateGameJSON(Request $request, NormalizerInterface $Normalizer,$id){
        $em=$this->getDoctrine()->getManager();
        $event=$em->getRepository(Stade::class)->find($id);

        $event->setNoms($request->get("noms"));
        $event->setNbrP($request->get("nbrP"));
