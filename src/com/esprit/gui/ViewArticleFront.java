 ...
 * @author ksaay
 */
public class ViewArticleFront extends Form{
     EncodedImage enc = null;
             boolean valid ;
Image imgs;
ImageViewer imgv;
String urli ;
 ArrayList<Interaction> listp;
    public ViewArticleFront(Form previous,Article ev){
        super(ev.getTitre());
           valid = false;
 ...
 // EncodedImage placeholder = EncodedImage.createFromImage(Image.createImage(mm * 6, mm * 8, 0), false);
          ...
          //   SpanLabel articlet = new SpanLabel(ev.getTitre(), "WalkthruWhite");
            ...
            Container tab1 = BorderLayout.centerAbsolute(BoxLayout.encloseY(
              ...
              articledesc,
                bottomSpace));
             this.add(tab1);
               listp=ServiceInteraction.getInstance().getAll(ev.getId(),ev);
    ...
    {
        valid = true;
        likeindex= likes.getIdi();
    }
               }
              
               if(valid)
               {
                   liked.setText("Dislike");
                   add(liked);
    ...
    public void actionPerformed(ActionEvent evt) {
        ...
        {
             System.out.print("Like");
              Interaction Comment= new Interaction("up","",MyApplication.u_c,ev);
              ServiceInteraction.getInstance().adlJson(Comment);
              liked.setText("Dislike");
              valid = true;
                //listp=ServiceInteraction.getInstance().getAll(ev.getId(),ev);
        }
        else
        {
         ...
         ServiceInteraction.getInstance().removeJL(ev);
        liked.setText("Like");
          ...
          TextField commentb = new TextField("","Your Comment");
          Button btnAdd = new Button("Add");
          commentBox.add(commentb).add(btnAdd);
          if(MyApplication.u_c.getId()!= 0 )
          add(commentBox);
    ...
    public void actionPerformed(ActionEvent evt) {
        ...
        else
        {
            ...
            Interaction Comment= new Interaction("comment",commentb.getText(),MyApplication.u_c,ev);
                if(ServiceInteraction.getInstance().addInteraction(Comment))
                {
...
for(Interaction c : listp) {
    if(c.getType().contains("comment".toLowerCase()))
    {
     ...
     new ViewArticleFrontE(this,ev,c).show();
});
